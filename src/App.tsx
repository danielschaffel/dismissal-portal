...
const StudentRow: React.FC<StudentRowProps> = ({ name, markPresent, markAbsent, isPresent}) => {
  ...
  <li className={`student-row ${isPresent ? 'present' : ''}`}>
      ...
      <button className="present-button" onClick={buttonAction}>{isPresent ? 'Mark Absent' : 'Mark Present'}</button>
    </li>
  );
};
...
const BusComponent: React.FC<BusComponentProps> = ({ routeName, students }) => { 
  ...
  const [studentData, setStudentData] = useState(
    students.map((student) => ({ name: student, isPresent: false, presentCount: 0, totalCount: students.length }))
  );
 ...
 const markStudentPresent = (index: number) => {
    setStudentData((prevStudentData) => {
      ...
      const presentCount = updatedStudentData.filter((student) => student.isPresent).length;
 ...
 const markStudentAbsent = (index: number) => {
  ...
  const toggleExpand = () => {
    ...
    const data = {
        ...
        datasets: [
            {
                data:[studentData[0].presentCount, studentData[0].totalCount - studentData[0].presentCount],
  ...
  return (
    <div>
      <h2 onClick={toggleExpand}>Bus Route: {routeName} ({studentData[0].presentCount} / {studentData[0].totalCount})
  ...
  // return <div>{JSON.stringify(data)}</div>;
    ...
    <div>
      ...
      {Object.entries(data).map(([routeName, students]) => (
        <BusComponent key={routeName} routeName={routeName} students={students as string[]} />
      ))}
    </div>
  );
